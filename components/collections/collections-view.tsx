...
const KIND_ICON: Record<CollectionKind, typeof Gamepad2> = {
...
const TITLE_LABELS: Record<CollectionKind, string> = {
  application: "Role or programme",
  info: "What to check",
  learning: "Skill or topic",
  book: "Book or paper",
  game: "Game",
  movie: "Movie or show",
  wish: "Something to buy",
};

const SOURCE_PLACEHOLDER: Record<CollectionKind, string> = {
  application: "Company or university",
  info: "Site, person, document",
  learning: "Coursera, YouTube",
  book: "Author",
  game: "Steam, PS5",
  movie: "Netflix, cinema",
  wish: "Amazon, local shop",
};
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await createCollectionItem({
        ...
        due_on: dueOn || null,
        price: SHOWS_PRICE.includes(kind) && price ? Number(price) : null,
      });
      if (!res.ok) {
...
{TITLE_LABELS[kind]}
        ...
        {/* One column relabelled per kind: the company for an application,
            the author for a book, where to watch for a film. */}
        {SOURCE_LABELS[kind] && (
          <div className="space-y-1.5">
            <Label htmlFor={`c-source-${kind}`} className="text-xs">
              {SOURCE_LABELS[kind]}
            </Label>
            <Input
              id={`c-source-${kind}`}
        ...
        {DUE_LABELS[kind] && (
          <div className="space-y-1.5">
            <Label htmlFor={`c-due-${kind}`} className="text-xs">
              {DUE_LABELS[kind]}
            </Label>
            <Input
              id={`c-due-${kind}`}
